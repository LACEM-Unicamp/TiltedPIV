...
for i = 1:length(angle)
    for j = 1:length(tec)
        ...
        if i == 1            
            Graph_RU(var_x,var_y,var_e,0,0,cmax,cmin,0,angle{i},showimpeller,tec{j},cam{j})
            title(column_title{j},'FontWeight','bold')
            ax=gca;
            ax.TitleHorizontalAlignment = 'left';
        elseif i == length(angle)
            Graph_RU(var_x,var_y,var_e,1,0,cmax,cmin,0,angle{i},showimpeller,tec{j},cam{j})
        else
            Graph_RU(var_x,var_y,var_e,0,0,cmax,cmin,0,angle{i},showimpeller,tec{j},cam{j})
        end
