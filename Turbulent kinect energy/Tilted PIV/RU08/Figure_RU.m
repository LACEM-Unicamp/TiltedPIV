...
for a = 1: length (met)
    for i = 1:length(angle)
        ...
        if (i == 1) && (a == length(met))         
            Graph_RU(var_x, var_y, var_e, 1, 1, cmax, cmin, 0, angle{i}, showimpeller, tec, cam)
            text (-0.25,0.45,'(b)','fontweight','bold')
        elseif i == 1
            Graph_RU(var_x, var_y, var_e, 0, 1, cmax, cmin, showtt, angle{i}, showimpeller, tec, cam)
            text (-0.25,0.45,'(a)','fontweight','bold')
        elseif a == 1
            Graph_RU(var_x, var_y, var_e, 0, 0, cmax, cmin, showtt, angle{i}, showimpeller, tec, cam)   
        elseif a == length(met)
